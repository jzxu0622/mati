...
for i_n=1:length(vin(:))
    ...
    R=mati.MULTIPRO.calMatrixR(timeStep,matrixSize,model.structure,beta);
    for i_b=1:model.pulse.Nacq
        ...
        Dex_t=model.pulse.b(i_b)*Dex(i_n)/(model.pulse.delta(i_b)+model.pulse.Delta(i_b));  
        s_ex=1-vin(i_n);
        A_0=mati.MULTIPRO.calMatrixA(model.pulse.gamma/2/pi*model.pulse.G(i_b)*timeStep/max(model.pulseArray.delta_q(i_b,:)),matrixSize,model.structure,beta); % A with the smallest q
        A_max=mati.MULTIPRO.calMatrixA(model.pulse.gamma/2/pi*model.pulse.G(i_b)*timeStep,matrixSize,model.structure,beta);
        
        for i=1:n_pulse
            if i==1
                S=mati.MULTIPRO.calMatrixS(model.pulseArray.q(i_b,i)*model.pulse.gamma/2/pi, matrixSize,model.structure,beta);       
                s_s=vin(i_n)*S;
                continue;
            end
            if i==n_pulse
                S=mati.MULTIPRO.calMatrixS(abs(model.pulseArray.q(i_b,i))*model.pulse.gamma/2/pi, matrixSize,model.structure,beta);
                S_nag=ctranspose(S);
                s_s=s_s*R*S_nag;
                continue;
            end            
            ...
            [s_in_t,s_ex]=mati.JOINT.generalSolution_blochEQ( Dinstant+kin(i_n),Dex_t+kex(i_n),kin(i_n),kex(i_n),s_in,s_ex,timeStep); 
