...
%               model.pulse.shape: pgse, tpgse, sin, cos, tcos, (tsin is under development)
...
%                 if any(D>3.07*1.1), warning('%s: D is larger than 110% of 3.07um^2/ms, the diffusion coefficient of free water at 37C', mfilename); end
    ...
    for n=1:length(pulse.shape)
        if ~ismember(pulse.shape(n), ["pgse"; "tpgse"; "sin"; "cos"; "tcos"] ) 
            error('%s: The #%d shape (%s) is not supported. A shape should be one of [pgse, tpgse, sin, cos, tcos]', mfilename, n, pulse.shape(n)); 
        end
    end
